  ...
  function sugerirReceita() {
    if (listaIngredientes.length === 0) {
      setReceitas([{
        ...
        titulo: 'Erro',
        tempoPreparo: 0,
        descricao: 'Por favor, adicione alguns ingredientes primeiro!'
      }]);
      return;
    }
    ...
    if (!tempo || parseInt(tempo) <= 0) {
      setReceitas([{
        ...
        titulo: 'Erro',
        tempoPreparo: 0,
        descricao: 'Por favor, informe um tempo válido!'
      }]);
      return;
    }

    const tempoDisponivel = parseInt(tempo);
    let sugestoes: Receita[] = [];

    if (tempoDisponivel <= 15) {
      sugestoes.push({
        id: '1',
        titulo: 'Omelete Rápido',
        tempoPreparo: 15,
        descricao: 'Um omelete delicioso com seus ingredientes disponíveis'
      });
    } else if (tempoDisponivel <= 30) {
      sugestoes.push({
        id: '1',
        titulo: 'Macarrão com Molho Rápido',
        tempoPreparo: 30,
        descricao: 'Macarrão al dente com molho especial'
      });
    } else {
      sugestoes.push({
        id: '1',
        titulo: 'Prato Elaborado',
        tempoPreparo: tempoDisponivel,
        descricao: 'Uma receita especial com seus ingredientes'
      });
    }

    setReceitas(sugestoes);
  }
            ...
            <View key={item.id} style={styles.receitaContainer}>
              ...
              {item.titulo !== 'Erro' && (
                <TouchableOpacity
                  ...
                  onPress={() => router.push('/passoapasso')}
                >
